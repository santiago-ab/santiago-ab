...
fn handle_add_item(list_item: ListItem, list_addr: HashString) -> ZomeApiResult<Address> {
    ...
    hdk::link_entries(&list_addr, &item_addr,"items".to_string(),"".to_string())?; // if successful, link to list address
    Ok(item_addr)
}
...
fn handle_get_list(list_addr: HashString) -> ZomeApiResult<GetListResponse> {
    ...
    let list_items = hdk::get_links(&list_addr, Some("items".to_string()),Some("".to_string()))?.addresses()
        .iter()
        .map(|item_address| {
